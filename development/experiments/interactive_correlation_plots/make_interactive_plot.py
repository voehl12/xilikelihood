...
def interactive_plot(correlation_values,ndim_values,prior, model,fiducial=5.0):
    # Initial parameters
    ...
    ndim = ndim_values[-1]
    datavector = model(fiducial, ndim=ndim)
    prior_model = model(prior, ndim=ndim)
    ...
    for ndim in ndim_values:
        ...
        for corr in correlation_values:
            ...
            # cov_varying: create covariance that changes with parameter for gamma likelihood
            posterior_vals = copula_likelihood(datavector, cov, prior_model)
            posterior_vals, mean_val = exp_norm_mean(prior,posterior_vals,reg=0)  # Normalize using integral
            max_val = prior[np.argmax(posterior_vals)]
            gaussian_posterior = multivariate_normal.logpdf(prior_model, mean=datavector, cov=cov)
    ...
    # Add sliders for ndim and correlation
    sliders = [
        {
            "steps": [
                {
                    "args": [
                        [f"ndim={ndim},corr={correlation_values[0]:.2f}"],
                        {"frame": {"duration": 300, "redraw": True}, "mode": "immediate"},
                    ],
                    "label": f"{ndim}",
                    "method": "animate",
                }
                for ndim in ndim_values[::-1]  # Reverse order for slider
            ],
            "currentvalue": {"prefix": "Number of Dimensions: ", "font": {"size": 16}},
            "x": 0.1,
            "xanchor": "left",
            "y": -0.2,  # Position the first slider higher
            "yanchor": "top",
        ...
        },
        {
            "steps": [
                {
                    "args": [
                        [f"ndim={ndim_values[-1]},corr={corr:.2f}"],
                        {"frame": {"duration": 300, "redraw": True}, "mode": "immediate"},
                    ],
                    "label": f"{corr:.2f}",
                    "method": "animate",
            ...
            "x": 0.1,
            "xanchor": "left",
            "y":-0.4,  # Position the second slider below the first
            "yanchor": "top",
        },
    ]
        ...
        updatemenus=[
            {
                ...
                "x": 0.1,
                "xanchor": "right",
                "y": -0.5,  # Position the button below the sliders
                "yanchor": "top",
            }
        ],
    )

    fig.write_html("interactive_plot_ndim_corr.html")
