import cov_calc 
import numpy as np
from helper_funcs import get_pp_sigma_n,get_global_sigma_n
import os.path
import healpy as hp
import wigner
...
def noise_cl(sigma2,len_l,nside):
    # separately for e,b and n?
    c_all = np.zeros((3,3,len_l))
    for i in range(3):
        c_all[i,i] = np.ones(len_l)
    return sigma2*hp.nside2pixarea(nside)*c_all
    #return sigma2*c_all


def cov_xi(cl_object=None,mask_object=None,pos_m=False,sigma_e=None,lmax=None):
    ...
    if sigma_e is not None:
        noise_sigma = get_global_sigma_n(1000,2.5*60,60.,sigma_e=sigma_e)
        #noise_sigma = get_pp_sigma_n(sigma_e=sigma_e)
        theory_cell += noise_cl(noise_sigma**2,lmax+1,mask_object.nside)
...
def cov_cl_gaussian(cl_object,sigma_e=None,nside=256,angle_in_arcmin=10,binsize_in_arcmin=30):
    cl_e = cl_object.ee.copy()
    if sigma_e is not None:
        
        noise_sigma = get_global_sigma_n(nside,1000,angle_in_arcmin/60,binsize_in_arcmin/60,sigma_e=sigma_e)
        cl_e += noise_sigma**2*hp.nside2pixarea(nside)*np.ones_like(cl_e)

    
    cl2 = np.square(cl_e)
    ell = np.arange(cl_object.lmax+1)
    diag = 2*cl2/(2*ell+1)
    return np.diag(diag), cl_e

def cov_xi_gaussian(cl_object,fsky,t1_in_deg,t2_in_deg,sigma_e=None,nside=None,binsize_in_arcmin=30):
    # e.g. https://www.aanda.org/articles/aa/full_html/2018/07/aa32343-17/aa32343-17.html
    
    c_lm, cl = cov_cl_gaussian(cl_object,sigma_e,nside,t1_in_deg*60,binsize_in_arcmin)
    lmax = cl_object.lmax
    wigners_p_l = wigner.wigner_dl(0,lmax,2,2,np.radians(t1_in_deg))
    
    wigners_p_m = wigner.wigner_dl(0,lmax,2,2,np.radians(t2_in_deg))
    l = m = 2 * np.arange(lmax+1) + 1
    lpart = wigners_p_l * l
    
    mpart = wigners_p_m * m
    norm = 1 / (4 * np.pi)
    
    mean = norm*np.sum(cl_object.ee*lpart)
    
    cov_xi = 1 / fsky * norm**2 * np.einsum('l,m,lm->',lpart,mpart,c_lm)
    return mean,cov_xi
